...
const JobRecommendationForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5001/api/career/predict-job-role",
        data
      );
      ...
      console.error("Error predicting job role or fetching career advice:", error);
      toast.error("An error occurred while fetching the job role or career advice."); // Show error toast
    } finally {
  ...
  return (
    <div>
      ...
      <p>
        Let us recommend a career path for you based on your skills, experience,
        and education.
      </p>
      ...
      <form onSubmit={handleSubmit} className="career-advisor-form">
        ...
        <Select
          ...
          value={experienceLevel ? { label: experienceLevel, value: experienceLevel } : null} // If no value, set to null
          onChange={(selected) => setExperienceLevel(selected ? selected.value : "")} // Properly handle null
        ...
        <Select
          ...
          value={educationLevel ? { label: educationLevel, value: educationLevel } : null} // If no value, set to null
          onChange={(selected) => setEducationLevel(selected ? selected.value : "")} // Properly handle null
      ...
      {predictedJobRole && (
        <div className="results-section">
          ...
          {careerAdvice && <p><strong>Career Advice:</strong> {careerAdvice}</p>}
        </div>
      )}
    </div>
  </div>
